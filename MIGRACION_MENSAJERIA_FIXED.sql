-- Migración: Sistema de Mensajería Directa (VERSIÓN CORREGIDA)
-- Fecha: 2025-01-06
-- Descripción: Agrega tablas y funciones para chat directo profesor-alumno y creador-jugador
...
-- Usa expresiones para ordenar los user IDs
...
-- ============================================================================
-- PASO 3: Renombrar tabla de adjuntos (si existe) o crear nueva genérica
-- ============================================================================

-- Verificar si existe ticket_attachments y renombrar
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ticket_attachments') THEN
        -- Backup de la tabla original
        EXECUTE 'CREATE TABLE IF NOT EXISTS ticket_attachments_backup AS SELECT * FROM ticket_attachments';

        -- Renombrar a nombre genérico
        ALTER TABLE ticket_attachments RENAME TO message_attachments;

        -- Agregar nueva columna para mensajes directos
        ALTER TABLE message_attachments ADD COLUMN IF NOT EXISTS direct_message_id INTEGER REFERENCES direct_messages(id) ON DELETE CASCADE;

        -- Actualizar constraint
        ALTER TABLE message_attachments DROP CONSTRAINT IF EXISTS check_attachment_reference;
        ALTER TABLE message_attachments ADD CONSTRAINT check_attachment_reference CHECK (
...
-- ============================================================================
-- PASO 7: Actualizar tabla de notificaciones para soportar mensajes directos
-- ============================================================================

-- Agregar columna para referencia a mensajes directos (si no existe)
DO $$
BEGIN
    IF NOT EXISTS (
...
BEGIN
    ...
    ELSE
        RAISE NOTICE '✅ Migración completada exitosamente. Todas las tablas fueron creadas.';
    END IF;
END $$;
